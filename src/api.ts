...
export const requestUsers = (params: Query = baseQuery): Promise<User[]> => {
    ...
    const filtered = users
        .filter((v) => {
            ...
            const passedAgeFilter = params.age === "" ? true : (params.age && v.age.toString().includes(params.age));
            const passedNameFilter = params.name === "" || (params.name && v.name.toLowerCase().includes(params.name.toLowerCase()));
